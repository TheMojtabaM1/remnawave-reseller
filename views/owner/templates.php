...
<div class="grid lg:grid-cols-3 gap-4">
  <div class="bg-card border border-line rounded-xl p-4 h-fit">
    ...
    <form method="post" action="/owner/templates" class="space-y-3">
      <?= csrf_field() ?>
      <input name="name" placeholder="نام قالب" required class="w-full bg-card2 border border-line2 rounded-lg px-3 py-2 text-sm">
  ...
  <div class="lg:col-span-2 space-y-3">
    <?php foreach ($templates as $t):
      ...
      <div class="bg-card border border-line rounded-xl p-4">
        <form method="post" action="/owner/templates/<?= $t['id'] ?>" class="grid md:grid-cols-5 gap-2 items-end">
          <?= csrf_field() ?>
          <div class="md:col-span-2"><label class="text-xs text-stone-400">نام</label><input name="name" value="<?= e($t['name']) ?>" class="w-full bg-card2 border border-line2 rounded-lg px-2 py-1.5 text-sm"></div>
          <div><label class="text-xs text-stone-400">حجم</label><input type="number" name="volume_gb" value="<?= (int)$t['volume_gb'] ?>" class="w-full bg-card2 border border-line2 rounded-lg px-2 py-1.5 text-sm"></div>
          <div><label class="text-xs text-stone-400">مدت</label><input type="number" name="duration_days" value="<?= (int)$t['duration_days'] ?>" class="w-full bg-card2 border border-line2 rounded-lg px-2 py-1.5 text-sm"></div>
          <div><label class="text-xs text-stone-400">HWID</label><input type="number" name="hwid_limit" value="<?= (int)$t['hwid_limit'] ?>" class="w-full bg-card2 border border-line2 rounded-lg px-2 py-1.5 text-sm"></div>
          ...
          <div class="md:col-span-5 flex gap-2 pt-1"><button class="bg-brand hover:bg-brand-light px-4 py-1.5 rounded-lg text-xs">ذخیره</button></div>
        </form>
    ...
    <?php endforeach; ?>
    <?php if (!$templates): ?><div class="bg-card border border-line rounded-xl p-6 text-center text-stone-500">قالبی تعریف نشده است.</div><?php endif; ?>
  </div>
</div>
