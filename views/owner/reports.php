<div class="flex items-center justify-between mb-4">
  ...
  <a href="/owner/reports/export" class="bg-emerald-600 hover:bg-emerald-500 px-4 py-2 rounded-lg text-sm">📊 خروجی Excel</a>
</div>
...
<div class="grid md:grid-cols-4 gap-3 mb-4">
  ...
  <div class="bg-sky-600 rounded-xl p-4"><div class="text-xs text-white/80">کانفیگ کل</div><div class="text-lg font-bold"><?= number_format($kpis['total_configs']) ?></div></div>
...
<div class="grid lg:grid-cols-2 gap-4 mb-4">
  <div class="bg-slate-900 border border-slate-800 rounded-xl p-4"><h3 class="font-semibold mb-3">روند درآمد</h3><canvas id="revChart" height="120"></canvas></div>
  <div class="bg-slate-900 border border-slate-800 rounded-xl p-4"><h3 class="font-semibold mb-3">پلن‌های پرفروش</h3><canvas id="planChart" height="120"></canvas></div>
</div>
...
<div class="bg-slate-900 border border-slate-800 rounded-xl p-4">
  ...
  <div class="overflow-x-auto">
    <table class="w-full text-sm">
      <thead class="text-slate-400 text-xs"><tr><th class="text-right p-2">نماینده</th><th class="text-right p-2">موجودی</th><th class="text-right p-2">کل فروش</th><th class="text-right p-2">بازگشت وجه</th><th class="text-right p-2">فروش خالص</th><th class="text-right p-2">کانفیگ</th></tr></thead>
      <tbody>
      <?php foreach ($perReseller as $row): $net = (int)$row['total_sales'] - (int)$row['total_refunds']; ?>
        <tr class="border-t border-slate-800">
          <td class="p-2"><a href="/owner/resellers/<?= $row['id'] ?>" class="text-sky-400"><?= e($row['display_name'] ?: $row['username']) ?></a></td>
      ...
      <?php endforeach; ?>
      <?php if (!$perReseller): ?><tr><td colspan="6" class="p-4 text-center text-slate-500">داده‌ای نیست.</td></tr><?php endif; ?>
      </tbody>
    </table>
  </div>
</div>
