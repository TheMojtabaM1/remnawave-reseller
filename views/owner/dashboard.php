...
$cards = [
    ...
    ['کانفیگ‌های فعال', number_format($kpis['active_configs']) . ' / ' . number_format($kpis['total_configs']), 'bg-sky-600'],
...
<div class="grid lg:grid-cols-3 gap-4">
  ...
  <div class="bg-slate-900 border border-slate-800 rounded-xl p-4">
    ...
    <?php if (!$alerts): ?>
      <p class="text-sm text-slate-500">هشداری وجود ندارد. ✅</p>
    <?php else: foreach ($alerts as $a): ?>
      <div class="text-sm border-b border-slate-800 py-2">
...
<div class="grid lg:grid-cols-2 gap-4 mt-4">
  <div class="bg-slate-900 border border-slate-800 rounded-xl p-4">
    ...
    <table class="w-full text-sm">
      <thead class="text-slate-400 text-xs"><tr><th class="text-right py-1">نماینده</th><th class="text-right">فروش</th></tr></thead>
      <tbody>
      <?php foreach ($topResellers as $r): ?>
        <tr class="border-t border-slate-800"><td class="py-2"><?= e($r['display_name'] ?: $r['username']) ?></td><td><?= toman((int) $r['sales']) ?></td></tr>
      <?php endforeach; ?>
      <?php if (!$topResellers): ?><tr><td colspan="2" class="text-slate-500 py-2">داده‌ای نیست.</td></tr><?php endif; ?>
      </tbody>
    </table>
  </div>
  ...
  <div class="bg-slate-900 border border-slate-800 rounded-xl p-4">
    ...
    <div class="space-y-1 text-sm max-h-72 overflow-y-auto">
      <?php foreach ($recent as $a): ?>
        <div class="border-b border-slate-800 py-1.5 flex justify-between">
          <span><span class="text-slate-400"><?= e($a['actor_name'] ?? $a['actor_type']) ?></span> — <?= e($a['action']) ?></span>
          <span class="text-xs text-slate-500"><?= jdate($a['created_at'], 'time') ?></span>
        </div>
      <?php endforeach; ?>
    </div>
  </div>
