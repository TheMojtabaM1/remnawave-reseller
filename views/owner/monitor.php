...
<div class="grid md:grid-cols-3 gap-3 mb-4">
  ...
  <div class="bg-brand rounded-xl p-5">
...
<div class="bg-card border border-line rounded-xl p-4">
  ...
  <table class="w-full text-sm">
    <thead class="text-stone-400 text-xs"><tr><th class="text-right p-2">نماینده</th><th class="text-right p-2">آنلاین</th></tr></thead>
    <tbody>
    <?php foreach ($perReseller as $rid => $count): $r = $resellers[$rid] ?? null; ?>
      <tr class="border-t border-line">
        <td class="p-2"><?php if ($r): ?><a href="/owner/resellers/<?= $rid ?>" class="text-brand"><?= e($r['display_name'] ?: $r['username']) ?></a><?php else: ?>#<?= $rid ?><?php endif; ?></td>
        <td class="p-2"><span class="bg-emerald-600/20 text-emerald-400 px-2 py-1 rounded text-xs"><?= (int)$count ?></span></td>
      </tr>
    <?php endforeach; ?>
    <?php if (!$perReseller): ?><tr><td colspan="2" class="p-4 text-center text-stone-500">در حال حاضر کاربر آنلاینی شناسایی نشد.</td></tr><?php endif; ?>
    </tbody>
  </table>
</div>
