...
function field($label, $name, $value, $type = 'number', $hint = '') {
    echo '<div><label class="block text-xs text-stone-400 mb-1">' . e($label) . '</label>';
    echo '<input type="' . $type . '" name="' . e($name) . '" value="' . $value . '" class="w-full bg-card2 border border-line2 rounded-lg px-3 py-2 text-sm">';
    if ($hint) echo '<div class="text-[11px] text-stone-500 mt-1">' . e($hint) . '</div>';
    echo '</div>';
}
?>
<form method="post" action="<?= $action ?>" class="space-y-6 max-w-4xl">
  ...
  <div class="bg-card border border-line rounded-xl p-4">
    ...
    <div class="grid md:grid-cols-3 gap-3">
      <div>
        <label class="block text-xs text-stone-400 mb-1">نام کاربری *</label>
        <input name="username" value="<?= $v('username') ?>" <?= $isEdit ? 'disabled' : 'required' ?> class="w-full bg-card2 border border-line2 rounded-lg px-3 py-2 text-sm disabled:opacity-60">
      </div>
      <div>
        <label class="block text-xs text-stone-400 mb-1">رمز عبور <?= $isEdit ? '(خالی = بدون تغییر)' : '*' ?></label>
        <input type="password" name="password" <?= $isEdit ? '' : 'required' ?> class="w-full bg-card2 border border-line2 rounded-lg px-3 py-2 text-sm">
      </div>
      <div>
        <label class="block text-xs text-stone-400 mb-1">پیشوند یوزرنیم *</label>
        <input name="prefix" value="<?= $v('prefix') ?>" <?= $isEdit ? 'disabled' : 'required' ?> placeholder="مثال: ali" class="w-full bg-card2 border border-line2 rounded-lg px-3 py-2 text-sm disabled:opacity-60">
      </div>
      <?php field('نام نمایشی', 'display_name', $v('display_name'), 'text'); ?>
      <?php field('شناسه تلگرام', 'telegram_id', $v('telegram_id'), 'text'); ?>
      <div>
        <label class="block text-xs text-stone-400 mb-1">انقضای دسترسی (UTC، خالی=نامحدود)</label>
        <input type="datetime-local" name="access_expires_at" value="<?= $isEdit && $reseller['access_expires_at'] ? e(str_replace(' ', 'T', substr($reseller['access_expires_at'],0,16))) : '' ?>" class="w-full bg-card2 border border-line2 rounded-lg px-3 py-2 text-sm">
      </div>
      <div>
        <label class="block text-xs text-stone-400 mb-1">وضعیت</label>
        <select name="status" class="w-full bg-card2 border border-line2 rounded-lg px-3 py-2 text-sm">
      ...
      <div class="md:col-span-3">
        <label class="block text-xs text-stone-400 mb-1">یادداشت</label>
        <textarea name="notes" rows="2" class="w-full bg-card2 border border-line2 rounded-lg px-3 py-2 text-sm"><?= $v('notes') ?></textarea>
      </div>
    </div>
  </div>

  <div class="bg-card border border-line rounded-xl p-4">
    <h3 class="font-semibold mb-3">محدودیت‌ها <span class="text-xs text-stone-500">(۰ = نامحدود)</span></h3>
  ...
  <div class="bg-card border border-line rounded-xl p-4">
    ...
    <p class="text-[11px] text-stone-500 mt-2">override‌های قیمت در صورت تعیین، بر قیمت پلن/سراسری ارجح‌اند. تغییر موجودی پس از ساخت، از صفحه نماینده انجام می‌شود.</p>
  </div>
  ...
  <div class="bg-card border border-line rounded-xl p-4">
    <h3 class="font-semibold mb-3">Squadهای مجاز <span class="text-xs text-stone-500">(خالی = همه مجاز)</span></h3>
    <?php if (!$squads): ?>
  ...
  <div class="flex gap-3">
    <button class="btn-brand px-6 py-2 rounded-lg font-semibold"><?= $isEdit ? 'ذخیره تغییرات' : 'ایجاد نماینده' ?></button>
    <a href="/owner/resellers" class="bg-white/10 hover:bg-white/20 px-6 py-2 rounded-lg">انصراف</a>
  </div>
</form>
